...
public class HomeController {
	...
	@Inject
	private IF_MemberService memberService; //사용자이름을 가지고오려고 추가
	...
	/**
	 * 스프링 security-context.xml설정한 로그인 처리 결과 확인
	 * @throws Exception 
	 * 
	 * 
	 * **/
	...
	public String login_success(Locale locale,HttpServletRequest request, RedirectAttributes rdat) throws Exception {
		...
		if (principal instanceof UserDetails) {
			//인증이 처리되는 로직(아이디 ,암호를 스프링시큐리티에 던져주고 인증은 스프링에서 알아서 해줌
		...
		if (enabled) { //인증처리가 완료된 사용자의 권한체크(아래)
			...
			session.setAttribute("session_userid", userid);//사용자ID
			session.setAttribute("session_levels", levels);//사용자권한
			//=========상단은 스프링시큐리티에서 기본제공 변수처리
			//========= 하단은우리가 추가하는 세션 변수처리
			//회원이름 구하기 추가

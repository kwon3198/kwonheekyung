...
public class PageVO {
	...
	private Integer page;//jsp단에서 null로 값이 올 때 에러가 발생하지 않도록 Integer 사용
	...
	private void calcPage() {
		...
		//시작 페이지 계산 클릭한 page번호 10일 때까지 시작페이지는 11
		if(tempEnd * this.perPageNum > this.totalCount) {
			//클릭한 page 번호로 계산된 게시물 수가 실제 게시물 갯수 totalCount 보다 클 때
			this.endPage = (int)Math.ceil(this.totalCount/(double)this.perPageNum);
		} else {
			//클릭한 page 번호로 계산된 게시물 수가 실제 게시물 갯수 totalCount 보다 작을 때
			this.endPage = tempEnd;
		}
		this.prev = this.startPage != 1;//시작페이지가 1보다 크면 무조건 이전 페이지가 있다.
		this.next = this.endPage * this.perPageNum < this.totalCount;
		//클릭한 페이지 번호로 계산된 게시물 수가 실제 게시물 갯수보다 작다면 다음페이지가 있음 true
	}
	...
	public void setTotalCount(int totalCount) {
		...
		calcPage(); //totalCount 전체게시물 갯수가 있어야지 페이지계산을 할 수 있기 때문에
	}
	...
	public int getStartNo() {
		//DB쿼리에서 사용하는 시작 데이터번호 = (jsp페이지번호-1)*페이지당 보여지는 갯수

...
public class PageVO {
	private int startNo;//게시판테이블,회원테이블의 필드 와는 직접관계없음.
	...
	private void calcPage() {
		...
		//ceil함수는 천장 함수로 1.1 = 2, 2.1 = 3으로 출력된다.
		//반대되는 바닥함수로 floor(), 반올림 함수로 round()가 있다.
		//jsp에서 클릭한 페이지번호를 기준으로 끝페이지를 계산한다.
		this.startPage = tempEnd - 9; 
		//시작페이지 계산 클릭한page번호10일때 까지 시작페이지는 1
		if(tempEnd * 10 > this.totalCount) {
			//클릭한 페이지번호로 계산된 게시물수가 totalCount클떄 
			this.endPage = (int)Math.ceil(this.totalCount/10.0);
		}else {
			//클릭한 페이지번호로 계산된 게시물수가 실제게시물개수 totalCount 작을때
			this.endPage = tempEnd;
		}
		this.prev = this.startPage != 1; //시작페이지 1보다 크면 무저건 이전페이지가 있음. true
		this.next = this.endPage * 10 < this.totalCount;
		//클릭한 pagd번호로 계산된 게시물수가 실제 게시물 개수보다 작다면 다음페이지가 있음  true
	}
